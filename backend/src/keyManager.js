/**
 * Sarvam API Key Pool Manager
 * 
 * Manages a DB-backed pool of API keys with in-memory caching.
 * When a key is exhausted (429), it's marked expired permanently
...
async function getActiveKey() {
  ...
  const result = await db.query(
    `SELECT api_key FROM sarvam_api_keys 
     WHERE status = 'active' 
  ...
  if (result.rows.length === 0) {
    throw new Error('ALL_KEYS_EXHAUSTED: No active Sarvam API keys remaining.');
  }

  cachedKey = result.rows[0].api_key;
  console.log(`[KeyManager] Active key loaded: ${maskKey(cachedKey)}`);
  return cachedKey;
}
...
async function markExpired(apiKey) {
  await db.query(
    `UPDATE sarvam_api_keys 
     SET status = 'expired', expired_at = NOW() 
     WHERE api_key = $1 AND status = 'active'`,
    [apiKey]
  );

  console.log(`[KeyManager] ❌ Key EXPIRED: ${maskKey(apiKey)}`);
